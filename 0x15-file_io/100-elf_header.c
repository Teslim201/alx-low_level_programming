 ...
 *
 * Description: print If the file is not an ELF file - exit code 98.
 */
void check_elf(unsigned char *e_ident)
{
	int index;
 ...
 * @e_ident: this is A pointer to an array containing the ELF magic numbers.
 *
 * Description: prints Magic numbers are separated by spaces.
 */
void print_magic(unsigned char *e_ident)
{
	int index;
 ...
 *
 * Description: print If the file cannot be closed - exit code 98.
 */
void close_elf(int elf)
{
	if (close(elf) == -1)
 ...
 *        ELF is the  header at the start of an ELF file.
 ...
 *
 * Description: print If the file is not an ELF File or
